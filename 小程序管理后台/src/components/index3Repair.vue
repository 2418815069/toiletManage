<template>
	<div>
		<div class="index3Repair" v-for="item in data">
			<div class="RepairHead">
				<span v-for="ite in Part(item.RepairsPart)">{{ite}}</span>
			...
			<p class="RepairMiaoshu">{{item.description}}</p>
			<div class="RepairPicture">
				<img v-for="(it,index) in Pictu(item.picture)" v-on:click="picturesZZc(item.picture,index)" :src="it"/>
				<p>{{data.tag=="未"?"未处理":"已处理"}}</p>
			</div>
		</div>
...
export default {
 	 ...
 	 data (){
 	  ...
 	  beforeCreate:function(){
    	var self = this;
    		var time=[];
    	 this.axios.post('admin/index3Repair',{
