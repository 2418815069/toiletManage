...
Page({
  ...
  onLoad: function(option) {
    ...
    amapInstance = new QQMapWX({
        key: 'OQNBZ-I64RO-MSTWF-SQIKD-PNUHS-XJBQ3' //此处使用你自己申请的key
       //key:'L6FBZ-YQ4CG-URYQY-IH36I-SXRFZ-IEBI5'
      }),
      // 设置地图的高度
      wx.getSystemInfo({
        success: function(res) {
          ...
          that.setData({
            ...
            syshigth: res.windowHeight - 150 - 40
          })
        }
      })
...
//标记点生成函数
  ...
  dingshiqi: function() {
   ...
   var numLat;
    var numSN=[];
    if (that.data.latandlon != ""){
      numLat = that.data.latandlon.slice(0,3);
      // console.log(numLat)
      for(var i=0;i<numLat.length;i++){
        if(numLat[i].sn==1||numLat[i].sn==0){
         
        }else{
          numSN.push({"sn":numLat[i].sn});
        }
      }
      ...
      if(numSN.length==0){
        ...
        // wx.request({
        //   url: 'http://119.23.237.30/personnel',
        //   method: 'post',
        //   data: {
        //     "sn": numSN
        //   },
        ...
        //           if(numLat[i].sn == res.data.data[j].sn){
        //             var list = {
        //               "man": res.data.data[j].male.staya,
        //               "woman": res.data.data[j].female.staya,
        //               "allman": res.data.data[j].male.pitNum,
        //               "allwoman": res.data.data[j].female.pitNum,
        //             }
        //             var str = "pagelist2[" + i + "]";
        //             that.setData({
        //               [str]: list
